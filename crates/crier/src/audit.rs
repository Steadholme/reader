...
//! worker.
//!
//! The note CONTENT never rides an event: producers fill only the logical fields (actor / action /
//! target=note-id / severity / detail). The audit records WHO posted WHICH note WHEN — never the
//! note body.
...
mod tests {
    ...
    fn disabled_sink_is_noop_and_never_drops() {
        ...
        for _ in 0..1000 {
            sink.emit(AuditEvent::info("crier.note.create", "u_a", "note_1", "ok"));
        }
    ...
    /// A note event serializes to exactly the safe shared fields — and NEVER the note body.
    #[test]
    fn note_event_serializes_without_content() {
        let ev = AuditEvent::info("crier.note.create", "u_alice", "note_42", "len=120");
        ...
        assert_eq!(v["action"], "crier.note.create");
        assert_eq!(v["target"], "note_42");
        assert_eq!(v["source"], "crier");
        for body in ["my secret post body", "hunter2"] {
    ...
    async fn emit_never_blocks_when_sink_unreachable() {
        ...
        for _ in 0..(QUEUE_CAPACITY * 8) {
            sink.emit(AuditEvent::info("crier.note.create", "u", "note_1", "d"));
        }
        assert!(
            sink.dropped() > 0,
