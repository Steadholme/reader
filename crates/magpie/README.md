...
Magpie 是 HOLDFAST 主权基础设施中的**稍后阅读（read-later）/ 网页剪藏（web clipper）**服务，部署于
...
服务遵循 HOLDFAST 共享模板：Rust + axum、async-trait 的 Postgres 存储层（内存 + PgStore，运行期 SQL、无
编译期宏、无 `block_in_place`）、独立数据库 `magpie`、幂等迁移、企业级 HOLDFAST UI、POST 接口的 CSRF 防护、
...
阅读列表页提供一个可拖拽的 **“Save to HOLDFAST”** 按钮。把它拖到浏览器书签栏后，在任意网页点击即可保存当前页。
