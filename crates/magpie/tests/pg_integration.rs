...
use magpie::model::{Clip, ClipQuery, Filter, SavedView};
...
async fn pg_store_full_integration() {
    ...
    let raw = PgPoolOptions::new()
        ...
        .await
        .unwrap();
    sqlx::query("DELETE FROM saved_views")
        .execute(&raw)
        .await
        .unwrap();
    ...
    assert!(store
        .create(&clip("aaaaaa11", "alice", "https://example.com/a", now))
        .await
        .unwrap());
    let got = store
        .get("aaaaaa11")
    ...
    assert!(store
        .find_by_owner_url("alice", "https://example.com/a")
        .await
        .unwrap()
        .is_some());
    assert!(store
        .find_by_owner_url("alice", "https://example.com/missing")
        .await
        .unwrap()
        .is_none());
    assert!(store
        .find_by_owner_url("bob", "https://example.com/a")
        .await
        .unwrap()
        .is_none());
    ...
    store
        .create(&clip("dddddd44", "bob", "https://example.com/d", now + 30))
        .await
        .unwrap();
    ...
    assert!(store
        .set_archived("cccccc33", "alice", false)
        .await
        .unwrap());
    ...
    assert_eq!(
        n, 2,
        "alice keeps bbbbbb22 + cccccc33 after deleting aaaaaa11"
    );
    ...
    assert!(store
        .set_tags(
            "bbbbbb22",
            "alice",
            magpie::model::normalize_tags("Rust, Web")
        )
        .await
        .unwrap());
    assert!(store
        .set_tags(
            "cccccc33",
            "alice",
            magpie::model::normalize_tags("gardening")
        )
        .await
        .unwrap());
    assert_eq!(
    ...
    assert!(store
        .list_by_tag("alice", "garden")
        .await
        .unwrap()
        .is_empty());
    let gardening = store.list_by_tag("alice", "gardening").await.unwrap();
    assert_eq!(
        gardening.iter().map(|c| c.id.as_str()).collect::<Vec<_>>(),
        vec!["cccccc33"]
    );
    ...
    // --- structured filters: list/list_by_tag identities + source AND filter ---
    let all_q = ClipQuery {
        filter: Filter::All,
        tag: None,
        site: None,
    ...
    assert_eq!(
        store.list_filtered("alice", &all_q, 100).await.unwrap(),
        store.list("alice", Filter::All).await.unwrap()
    );
    let rust_q = ClipQuery {
        filter: Filter::All,
        tag: Some("rust".into()),
        site: None,
    };
    assert_eq!(
        store.list_filtered("alice", &rust_q, 100).await.unwrap(),
        store.list_by_tag("alice", "rust").await.unwrap()
    );
    let source_q = ClipQuery {
        filter: Filter::All,
        tag: Some("rust".into()),
    ...
    assert_eq!(
        store
            .list_filtered("alice", &source_q, 100)
            .await
            .unwrap()
            .iter()
            .map(|c| c.id.as_str())
    ...
    let cur = magpie::model::Cursor {
        saved_at: p1[0].saved_at,
        id: p1[0].id.clone(),
    };
    let p2 = store
        .search("alice", "second line", Some(&cur), 10)
        .await
        .unwrap();
    assert_eq!(
    ...
    let by_title = store
        .search("alice", "title bbbbbb22", None, 10)
        .await
        .unwrap();
    assert_eq!(
        by_title.iter().map(|c| c.id.as_str()).collect::<Vec<_>>(),
        vec!["bbbbbb22"]
    );
    assert!(store
        .search("bob", "title bbbbbb22", None, 10)
        .await
        .unwrap()
        .is_empty());

    // --- highlights: add / list / dedup+note / delete, owner-scoped ---------
    sqlx::query("DELETE FROM highlights")
        .execute(&raw)
        .await
        .unwrap();
    ...
    assert!(store
        .add_highlight(&hl("hl000001", "bbbbbb22", "alice", "alpha", 100))
        .await
        .unwrap());
    assert!(store
        .add_highlight(&hl("hl000002", "bbbbbb22", "alice", "beta", 200))
        .await
        .unwrap());
    assert!(store
        .add_highlight(&hl("hl000003", "cccccc33", "alice", "gamma", 300))
        .await
        .unwrap());
    assert!(store
        .add_highlight(&hl("hl000004", "bbbbbb22", "bob", "delta", 400))
        .await
        .unwrap());
    // id-level idempotency: re-inserting the same id is a no-op.
    assert!(!store
        .add_highlight(&hl("hl000001", "bbbbbb22", "alice", "changed", 999))
        .await
        .unwrap());
    ...
    assert_eq!(
        margin.iter().map(|h| h.id.as_str()).collect::<Vec<_>>(),
        vec!["hl000001", "hl000002"]
    );
    ...
    assert_eq!(
        ...
        vec!["hl000003", "hl000002", "hl000001"]
    );
    assert_eq!(store.list_all_highlights("bob").await.unwrap().len(), 1);

    // dedup lookup + note edit (nullable column round-trips).
    assert!(store
        .find_highlight_by_quote("alice", "bbbbbb22", "alpha")
        .await
        .unwrap()
        .is_some());
    assert!(store
        .find_highlight_by_quote("alice", "bbbbbb22", "nope")
        .await
        .unwrap()
        .is_none());
    assert!(store
        .set_highlight_note("hl000001", "alice", Some("a note".into()))
        .await
        .unwrap());
    assert_eq!(
        store
            .get_highlight("hl000001")
            .await
            .unwrap()
            .unwrap()
            .note
    ...
    assert!(store
        .set_highlight_note("hl000001", "alice", None)
        .await
        .unwrap());
    assert!(store
        .get_highlight("hl000001")
    ...
    assert!(!store
        .set_highlight_note("hl000001", "bob", Some("x".into()))
        .await
        .unwrap());
    ...
    // --- saved views: insert/list/get/delete, owner-scoped ----------------------
    let view = SavedView {
        id: "view0001".into(),
    ...
    assert!(store.create_saved_view(&view).await.unwrap());
    assert!(!store.create_saved_view(&view).await.unwrap());
    assert_eq!(
        store
            .list_saved_views("alice")
            .await
            .unwrap()
            .iter()
            .map(|v| v.id.as_str())
    ...
    assert!(!store.delete_saved_view("view0001", "bob").await.unwrap());
    assert!(store.delete_saved_view("view0001", "alice").await.unwrap());
    assert!(store.get_saved_view("view0001").await.unwrap().is_none());
    ...
    assert_eq!(
        ex.iter().map(|c| c.id.as_str()).collect::<Vec<_>>(),
        vec!["cccccc33", "bbbbbb22"]
    );
    ...
    assert_eq!(
        exh.iter().map(|h| h.id.as_str()).collect::<Vec<_>>(),
        vec!["hl000002", "hl000003"]
    );
